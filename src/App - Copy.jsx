...
export default function App() {
    ...
    try {
      Promise.resolve().then(async () => {
         ...
         const itemsToSet = resolvedArray.filter(item => {
           const oldItem = oldArray.find(o => String(o.id) === String(item.id));
         ...
         let batch = writeBatch(db);
         let count = 0;
         for (const item of itemsToDelete) { batch.delete(doc(db, collectionName, String(item.id))); count++; if (count === 400) { await batch.commit(); batch = writeBatch(db); count = 0; } }
         for (const item of itemsToSet) { batch.set(doc(db, collectionName, String(item.id)), JSON.parse(JSON.stringify(item))); count++; if (count === 400) { await batch.commit(); batch = writeBatch(db); count = 0; } }
  ...
  const customSetStoreInfo = async (next) => { const res = typeof next === 'function' ? next(stateRef.current.storeInfo) : next; setStoreInfo(res); try { await setDoc(doc(db, "settings", "storeInfo"), JSON.parse(JSON.stringify(res))); } catch (e) {} };
  const customSetCategories = async (next) => { const res = typeof next === 'function' ? next(stateRef.current.categories) : next; setCategories(res); try { await setDoc(doc(db, "settings", "categories"), { values: JSON.parse(JSON.stringify(res)) }); } catch (e) {} };
  const customSetUnits = async (next) => { const res = typeof next === 'function' ? next(stateRef.current.units) : next; setUnits(res); try { await setDoc(doc(db, "settings", "units"), { values: JSON.parse(JSON.stringify(res)) }); } catch (e) {} };

  const showToast = (msg, type = 'success') => { setToast({ msg, type }); setTimeout(() => setToast(null), 3000); };

  const handleRestoreDatabase = async (parsedData) => {
    showToast("Mengunggah data ke Cloud, mohon tunggu...", "success");
    try {
      ...
      showToast("Data sukses direstore ke Cloud!", "success");
    } catch (err) { showToast("Gagal sinkronisasi", "error"); }
  };
  ...
  return (
    <div className={`flex h-screen w-full ${themeColors.bg} ${themeColors.text} overflow-hidden`}>
