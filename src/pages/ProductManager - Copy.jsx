...
export default function ProductManager({ products, setProducts, categories, units, colors, showToast, user, isSoundOn }) {
  ...
  const defaultForm = { barcode: '', name: '', category: categories[0] || '', unit: units[0] || '', stock: 0, cost: 0, price: 0, img: '📦' };
        ...
        try {
          ...
          if (text.includes('<table')) {
            ...
            for (let i = 1; i < rows.length; i++) {
              ...
              const unit = cols[1]?.textContent?.trim() || units[0];
              ...
              const category = cols[6]?.textContent?.trim() || categories[0];
            ...
            for (let i = 1; i < lines.length; i++) {
              ...
              const unit = cols[1]?.trim() || units[0];
              ...
              const category = cols[6]?.trim() || categories[0];
