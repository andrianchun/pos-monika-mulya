...
export default function Reports({ sales, purchases, products, accounting, setAccounting, financialAccounts, colors, isSoundOn, theme, storeInfo, showToast }) {
  ...
  const chartInfo = useMemo(() => {
     ...
     if (activeDataArray.length === 0) {
        if (filterMode === 'Hari Ini') labels = ['08:00', '12:00', '16:00', '20:00'];
        else if (filterMode === 'Minggu Ini') labels = ['Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab', 'Min'];
        else if (filterMode === 'Bulan Ini') labels = Array.from({length: new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate()}, (_, i) => String(i + 1));
        else if (filterMode === 'Tahun Ini') labels = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Ags', 'Sep', 'Okt', 'Nov', 'Des'];
     ...
     if (filterMode === 'Hari Ini' || (filterMode === 'Manual' && manualDiffDays <= 1)) {
     ...
     else if (filterMode === 'Bulan Ini' || (filterMode === 'Manual' && manualDiffDays <= 31)) {
     ...
     else if (filterMode === 'Tahun Ini' || (filterMode === 'Manual' && manualDiffDays <= 365)) {
        if (filterMode === 'Tahun Ini') {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div onClick={() => { playSound('pop', isSoundOn); setActiveReport('pembelian'); }} className={`p-6 rounded-2xl border ${colors.border} ${colors.panel} shadow-sm flex flex-col items-center justify-center text-center h-48 cursor-pointer border-blue-500 hover:shadow-blue-500/20 transition-all group`}><Package size={48} className={`mb-3 ${colors.gold} group-hover:scale-110 transition-transform`} /><h3 className={`font-bold text-lg ${colors.text}`}>Laporan Pembelian</h3><p className={`text-sm ${colors.textMuted}`}>History kulakan, hutang supplier, nilai aset stok.</p></div>
          <div onClick={() => { playSound('pop', isSoundOn); setActiveReport('neraca'); }} className={`p-6 rounded-2xl border ${colors.border} ${colors.panel} shadow-sm flex flex-col items-center justify-center text-center h-48 cursor-pointer border-purple-500 hover:shadow-purple-500/20 transition-all group`}><PieChart size={48} className={`mb-3 ${colors.gold} group-hover:scale-110 transition-transform`} /><h3 className={`font-bold text-lg ${colors.text}`}>Neraca Keuangan</h3><p className={`text-sm ${colors.textMuted}`}>Pembukuan, aset, liabilitas & ekuitas toko.</p></div>
        </div>
      </div>
    );
  }
             ...
             <div>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                   <div className={`lg:col-span-2 p-4 border rounded-xl ${colors.border} bg-gray-50 dark:bg-[#2a2a24]`}>
                      ...
                      <SimpleChart chartData={chartInfo.data} chartType={activeReport === 'pembelian' ? 'Pembelian' : 'Penjualan'} chartMode={chartMode} labels={chartInfo.labels} theme={theme} colors={colors} heightClass="h-72"/>
                   </div>
                         ...
                         <div className="space-y-2">
                            {topItems.map((item, i) => (
                              <div key={i} className={`flex justify-between items-center p-2 rounded bg-white dark:bg-[#1e1e1e] border ${colors.border} text-sm shadow-sm`}>
                                <span className={`font-semibold line-clamp-1 ${colors.text}`}>{i+1}. {item.name}</span><span className={`font-bold ${activeReport === 'penjualan' ? colors.gold : '${colors.gold}'}`}>{item.total}x</span>
                              </div>
                            ))}
             ...
             <div>
                ...
                <div className="mt-8 space-y-4">
                   ...
                   <DataTable title="Rincian Jurnal Pembukuan" columns={accColumns} data={filteredAccountingTable} defaultSort={{key: 'date', direction: 'desc'}} colors={colors} actions={[
                     { icon: Edit, label: 'Edit', colorClass: 'bg-stone-200 text-stone-700 dark:bg-stone-700 dark:text-stone-200 hover:bg-blue-200', onClick: (a) => { playSound('pop', isSoundOn); setAccForm({...a, amount: Math.abs(a.amount).toString(), isExpense: a.amount < 0, date: new Date(a.date).toISOString().split('T')[0] }); setShowAccModal(true); } }
             ...
             <div className={`w-full max-w-lg p-6 rounded-2xl shadow-2xl ${colors.panel} border ${colors.border}`}>
               ...
               <form onSubmit={handleAccSave} className="space-y-4">
                 <div>
                   ...
                   <input type="date" required className={`w-full p-3 rounded-xl border ${colors.border} bg-transparent ${colors.text} outline-none focus:ring-1 focus:ring-[#D4AF37] [color-scheme:light] dark:[color-scheme:dark]`} value={accForm.date} onChange={e=>setAccForm({...accForm, date: e.target.value})} />
                 </div>
                 <div>
