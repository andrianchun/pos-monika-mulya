...
// PERBAIKAN: Menerima props setCustomers
export default function POS({ products, setProducts, customers, setCustomers, suppliers, sales, setSales, purchases, setPurchases, colors, showToast, user, isSoundOn, theme, storeInfo, setStoreInfo, accounting, setAccounting, financialAccounts }) {
  ...
  // Perbaikan ID fallback umum adalah 1 atau string '1'
  ...
  const handleCheckout = (e, action) => {
    ...
    // PERBAIKAN: Menambahkan poin ke database Customer!
    ...
    if (paid > 0 && activeAccount) setAccounting([...accounting, { id: Date.now()+1, type: 'kas', accountId: activeAccount.id, name: posMode === 'penjualan' ? `Penerimaan Nota ${genNota}` : `Pembayaran Nota ${genNota}`, amount: posMode === 'penjualan' ? paid : -paid, date: docDate.toISOString() }]);
    if(posMode === 'penjualan') setSales(prev => [newRecord, ...prev]); else setPurchases(prev => [newRecord, ...prev]);
    
    setCart([]); setPosDiscountStr(''); setPosOngkirStr(''); setCheckoutModal(false); setShowMobileCart(false); setPaymentAmount(''); setDueDate(''); setTransactionDate(''); setUseAutoOngkir(false);
    
    if (action === 'simpan') {
       setCompletedDoc(null);
       showToast(`Transaksi Berhasil Tersimpan! ${earnedPoints > 0 ? `(+${earnedPoints} Poin Ditambahkan ke ${cName})` : ''}`, 'success');
          ...
          <div className={`w-full lg:w-[400px] xl:w-[450px] flex-col ${colors.panel} shrink-0 h-full border-t lg:border-t-0 shadow-lg lg:shadow-none z-10 ${showMobileCart ? 'flex' : 'hidden lg:flex'}`}>
            ...
            <div className={`p-3 sm:p-4 border-b ${colors.border} flex justify-between items-center bg-gray-50 dark:bg-[#2a2a24] shrink-0`}>
              <div className="flex-1 mr-2 relative">
                 ...
                 <SearchableSelect options={posMode === 'penjualan' ? customers : suppliers} value={posMode === 'penjualan' ? selectedCustomer : selectedSupplier} onChange={posMode === 'penjualan' ? setSelectedCustomer : setSelectedSupplier} placeholder={`Pilih ${posMode === 'penjualan' ? 'Customer' : 'Supplier'}...`} colors={colors} />
