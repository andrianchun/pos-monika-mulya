...
export default function Reports({ sales, purchases, products, accounting, setAccounting, financialAccounts, colors, isSoundOn, theme, storeInfo, showToast }) {
  ...
  const generateChartData = (dataArray) => {
     let points = filterMode === 'Hari Ini' ? 7 : filterMode === 'Minggu Ini' ? 7 : filterMode === 'Bulan Ini' ? 4 : 12;
     if(filterMode === 'Keseluruhan' || filterMode === 'Manual') points = 10;
     let baseData = Array.from({length: points}).map(() => Math.max(10, Math.floor(Math.random() * 100)));
     if(dataArray.length > 0) {
        const maxData = dataArray.reduce((max, s) => s.total > max ? s.total : max, 10);
        return dataArray.slice(0, points).map(s => ({ value: s.total, percentage: (s.total / maxData) * 100 }));
     }
     const maxVal = Math.max(...baseData, 10); return baseData.map(val => ({ value: val * 100000, percentage: (val / maxVal) * 100 }));
  }
  const repChartLabels = () => {
      ...
      if (filterMode === 'Bulan Ini') return ['Mg 1', 'Mg 2', 'Mg 3', 'Mg 4'];
